...
// Write down a given Assembly Line and its cost into an output file.
void process (const vector<int>& solution, int c);
// Compute the cost of an Assembly Line given a range and an Upgrade:
int compute_cost (const vector<int>& line, int upgrade, int lower_bound, int upper_bound);
// Compute the penalization for a given Upgrade:
int penalization (const vector<int>& line, int upgrade, int index);
// Compute the proability + Boltzmann Distribution:
double probability (int cost, int new_cost, double T);
// Compute the cost of a given Assembly Line:
int cost (const vector<int>& solution);
// Generate an initial random Assembly Line:
vector<int> generate_initial_solution(int n);
// Choose a random neighbour of the given Assembly Line:
vector<int> random_neighbour_of (const vector<int>& solution);
// Perform a Local Search technique with Simulated Annealing:
vector<int> local_search (const vector<int>& solution, double& T0);
// Generate an Assembly Line + Simulated Annealing initial conditions:
...
/* RESULTS FOR STRATEGIES
 ...
 meta has randomness. Executed several times and the best result has been chosen.
 */
